...
class SegmentationFolder(PairedDataset):
    ...
    def _setup_datasets(self):
        ...
        if self.config.get('inferer') is not None:
            ...
            eval_batch_size = 1
            warn(f"Using custom inferer {self.config.inferer.get('_target_', 'unknown')}, setting eval batch size to 1. Inferer may handle batching. ")
        else:
    ...
    def _get_transform(self, transform: dict):
        ...
        match transform.type:
            ...
            case 'EnsureChannelFirst':
                return EnsureChannelFirstd(
                    keys=[self.image_key, self.label_key],
                    channel_dim=transform.channel_dim
                )
            case _:
    ...
    def _create_dict(self, image_file):
        output = { 
            self.image_key: os.path.join(self.images_dir, image_file)
        }
